...
<div class="col-lg-12 col-md-12">
	...
	<div class="row">
		<nav class="navbar">
			<div class="container-fluid">
				<form class="navbar-form navbar-right" action="/lar/admin/commu/qa/search" method="post">
					...
					<div class="form-group">
						<input type="text" class="form-control" placeholder="Search" name="text" value="" required/>
	...
	<div class="row table-responsive">
		<table class="table table-striped table-condensed">
			...
			<c:forEach items="${commu_list}" var="cq">
				<tr>
					<td>${cq.commu_Index}</td>
					<td>
						<p><a href="/lar/admin/commu/view/${cq.commu_Index}">${cq.commu_Title}</a></p>
						<c:forTokens items="${cq.commu_tags}" var="tag" delims=",">
							<a href="/lar/admin/commu/qa/search/tag/${tag}/list/1" class="bg-warning labelinput badge badge-warning" style="background-color: #fed136; color: #4c0b5f; font-size: 10px;">#${tag}</a>
						</c:forTokens>
					</td>
			...
			<ul class="pagination">
				...
				<% for (int i=pi.getStart_page(); i<=pi.getEnd_page(); i++) { %>
					<% if (i == pi.getCurrent_page()) { %>
						<li class="active"><a href="/lar/admin/commu/qa/list/<%=i%>" onclick="return false;"><%=i%></a></li>
